...
export class HealerPlayer extends Player {
    constructor(idleTextures : Texture[],
        ...
        super(idleTextures, runTextures, origin, 0.1, 50, 8, 10, 0.025);
    ...
    public update(delta : number, boss : Boss,  players : Player[], colliders : CollisionBox[]) : void {
        ...
        for (let i = 0; i < this.projectiles.length; i++) {
            ...
            } else if (bullet.colBox.doesCollideWith(players[0].colBox)) {
                bullet.play();

                if (bullet.didAnimFinish()) {
                    this.projectiles[i].destroy();
                    this.projectiles.splice(i, 1);
                    players[0].heal(this.healPower);
                }
    ...
    public override throwHeal(renderer : Renderer, player : Player) : number {
        if (this.mana <= this.abilityCost - 1)
            return;
